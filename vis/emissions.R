...
data2$Launch.Event <- as.numeric(gsub(",", "", data2$Launch.Event))
data2$Launcher.Production <- as.numeric(gsub(",", "", data2$Launcher.Production))
data2$Electronics.Production <- as.numeric(gsub(",", "", data2$Electronics.Production))
data2$Launcher.Transportation <- as.numeric(gsub(",", "", data2$Launcher.Transportation))
data2$Electricity.Consumption <- as.numeric(gsub(",", "", data2$Electricity.Consumption))
data2$Propulsion.System <- as.numeric(gsub(",", "", data2$Propulsion.System))
...
d_per_subscriber_emissions <-
  ...
  geom_text(data = total_emissions2, aes(x = Constellation, y = total_emission_value, label = comma(total_emission_value)), vjust = -0.5, size = 3)
...
path_combined = file.path(visualizations, 'f_emissions.png')
